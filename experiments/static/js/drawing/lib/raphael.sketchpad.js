...
var t_previous = Date.now();
var t_counter;
// var did_undo = false; // to track whether an undo was used.
var DOSNAP = false;
...
(function(Raphael) {
	...
	var SketchPad = function(paper, options) {
		...
		self.check_if_did_undo = function() {
			return did_undo;
		};
		...
		function _redraw_strokes() {
			...
			for (var i = 0, n = _strokes.length; i < n; i++) {
				var stroke = _strokes[i];
				var type = stroke.type;
				if (stroke.primitive=="circle") {
					_paper["circle"](stroke.circle[0], stroke.circle[1], stroke.circle[2])
						.attr(stroke)
						.click(_pathclick);					
				} else {
					_paper[type]()
		...
		function _mousemove(e) {
			if (_drawing==true) {
				...
				if (Date.now() - t_previous > 60 && t_counter>5) {
					console.log("new stroke")
					if (DOSNAP==true) {
						_mouseup(e); // then consider this the start of a new stroke. act like picked up pen here, and then placed right back down.
						_mousedown(e);						
					}
				}				
				t_previous = Date.now();
		...
		function _mouseup(e) {
			...
			if (path != null) {
				...
				stroke.pathorig = path.pathorig;
				stroke.primitive = path.primitive;
				stroke.path_nostring = path.attrs.path;
				stroke.circle = [];

				if (DOSNAP==true) {
					if (stroke.primitive=="circle") {
						stroke.circle = [sketchpad.paper().top[0].attributes.cx.nodeValue, sketchpad.paper().top[0].attributes.cy.nodeValue, sketchpad.paper().top[0].attributes.r.nodeValue];
						// stroke.path_nostring = [];
					} 					
				}
	...
	var Pen = function() {
		...
		self.finish = function(e, sketchpad) {
			...
			if (_c != null) {
				if (_points.length <= 1) {
					...
					// replaces real stroke with line between start and end
					primitive = get_primitive_type(_points);

					if (primitive=="line") {
						_pointssimple.push(_points[0]);
						_pointssimple.push(_points.slice(-1)[0]);
						pathsimple_svg = points_to_svg_2(_pointssimple);
						if (DOSNAP==true) {
							_c.attr({ path: pathsimple_svg });	
						}
		...
		function get_primitive_type(_points) {
		// figures out whether points should be line or circle
			...
			for (i = 0; i < _points.length; i++) {
				xvals.push(_points[i][0]);
				yvals.push(_points[i][1]);
			}		

			xmin = Math.abs(xvals[0]-xvals.slice(-1)[0]);
			xmax = Math.abs(Math.max(...xvals) - Math.min(...xvals));

			ymin = Math.abs(yvals[0]-yvals.slice(-1)[0]);
			ymax = Math.abs(Math.max(...yvals) - Math.min(...yvals));

			if (xmin<2*xmax/3 && ymin<2*ymax/3 && xmax>3 && ymax>3 && xmax>ymax/3 && ymax>xmax/3) {
				// i.e. if loops around in both x and y axes, and not too small, and y and x deviations are similar.
				primitive="circle";
				if (DOSNAP==true) {
					_x = mean(xvals);
					_y = mean(yvals);
					...
					for (i = 0; i < xvals.length; i++) {
						_tmp.push(Math.hypot(xvals[i] - _x, yvals[i] - _y));
					};		
					_radius = mean(_tmp);
					sketchpad.paper().circle(_x, _y, _radius).attr({
						stroke: _color, 
						"stroke-opacity": _opacity,
						"stroke-width": _width,
					});					
				}
				// sketchpad.paper().circle(_x, _y, _radius);
			} else {
				primitive="line";
			}
			return primitive;
		...
		self.move = function(e, sketchpad) {
			if (_drawing == true) {
				var x = e.pageX - _offset.left,
					y = e.pageY - _offset.top;
					t = Date.now()		
				if (t-_times.slice(-1)[0]>100 && _times.length>2) {
					console.log("new stroke!")
				}	
				_points.push([x, y]);
				_times.push(t)
		...
		function points_to_svg_2(_points2) {
			if (_points2 != null && _points2.length > 1) {
				var p = _points2[0];
				var path = "M" + p[0] + "," + p[1];
				for (var i = 1, n = _points2.length; i < n; i++) {
					p = _points2[i];
					path += "L" + p[0] + "," + p[1]; 
				} 
				return path;
