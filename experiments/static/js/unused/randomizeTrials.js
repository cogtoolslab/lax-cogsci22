var generateConditionSequence = function (
  structureList,
  ...
  var miniBlockTemplate = repeatArray(
    conditions,
    miniblock_size / conditions.length
  );
  ...
  while (!found_good_sequence) {
    ...
    comsEqual =
      centerOfMass(candidateSequence, conditions[0]) ==
      centerOfMass(candidateSequence, conditions[1]);
...
var setupRandomTrialList = function (
  trialTemplates,
  conditions = ["mental", "physical"],
  miniblock_size = 4
) {
  conditions = generateConditionSequence(
    trialTemplates,
    (conditions = ["mental", "physical"]),
    (miniblock_size = 4)
  );
...
var shuffle = function (listToShuffle) {
  listToShuffle.sort(function (a, b) {
...
var prePostStimList = function (
  ...
  // added iteratively)
  commonRepeatedTrialInfo = {
    targetIDs: repeatedTargets,
    condition: "repeated",
  };

  commonControlTrialInfo = { targetIDs: controlTargets, condition: "control" };

  // pre phase
  var pre = _.shuffle(
    _.concat(
      _.map(repeatedTargets, (curTarget) => {
        return _.extend({}, commonRepeatedTrialInfo, structureList[curTarget], {
          phase: "pre",
          repetition: 0,
          targetID: curTarget,
        });
      ...
      _.map(controlTargets, (curTarget) => {
        return _.extend({}, commonControlTrialInfo, structureList[curTarget], {
          phase: "pre",
          repetition: 0,
          targetID: curTarget,
        });
